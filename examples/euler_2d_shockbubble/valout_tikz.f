...
c        ###  make the file names and open output files
         ...
         do 55 ipos = 10, 7, -1
            ...
            fname5(ipos-1:ipos-1) = char(ichar('0') + idigit)
            nstp = nstp / 10
 55      continue
...
c        # From here, everything is good
 ...
 1009    format('%',/,
     ...
     &         '%       {\stepcounter{level}',/,
     &         '%       \includegraphics',/,
     &         '%       {amr_sb_',I0.4,'_\thelevel.png}}',/,
     &         '%\begin{document}',/,
     &         '%\input{tikz.',I0.4,'.tex}',/,
     &         '%\end{document}',/,
     &         '%',/,
     &         '% This expects level figures amr_sb_',I0.4,'_01.png,',/,
     &         '% amr_sb_',I0.4,'_02.png - one per level. ',/,
...
c65      if (level .gt. lfine) go to 90
 65      if (level .gt. lend) go to 90
            mptr = lstart(level)
            write(matunit5,1012) level,mxf_global/2.d0,
     &            myf_global/2.d0
...
c            # NOTE: we are writing out ghost cell data also, unlike ascii
 ...
 90     continue
        ...
        write(matunit5,'(A)') '\end{tikzpicture}'
        close(matunit5)
